export const GET_CARS_REQUEST = 'GET_CARS_REQUEST';
export const GET_CARS_SUCCESS = 'GET_CARS_SUCCESS';
export const GET_CARS_FAIL = 'GET_CARS_FAIL';

export const GET_CAR_REQUEST = 'GET_CAR_REQUEST'
export const GET_CAR_SUCCESS = 'GET_CAR_SUCCESS'
export const GET_CAR_FAIL = 'GET_CAR_FAIL'

export const ADD_CAR_REQUEST = 'ADD_CAR_REQUEST'
export const ADD_CAR_SUCCESS = 'ADD_CAR_SUCCESS'
export const ADD_CAR_FAIL = 'ADD_CAR_FAIL'
export const ADD_CAR_RESET = 'ADD_CAR_RESET'

export const EDIT_CAR_REQUEST = 'EDIT_CAR_REQUEST'
export const EDIT_CAR_SUCCESS = 'EDIT_CAR_SUCCESS'
export const EDIT_CAR_FAIL = 'EDIT_CAR_FAIL'
export const EDIT_CAR_RESET = 'EDIT_CAR_RESET'

export const DELETE_CAR_REQUEST = 'DELETE_CAR_REQUEST'
export const DELETE_CAR_SUCCESS = 'DELETE_CAR_SUCCESS'
export const DELETE_CAR_FAIL = 'DELETE_CAR_FAIL'
export const DELETE_CAR_RESET = 'DELETE_CAR_RESET'

export const BOOK_CAR_REQUEST = 'BOOK_CAR_REQUEST'
export const BOOK_CAR_SUCCESS = 'BOOK_CAR_SUCCESS'
export const BOOK_CAR_FAIL = 'BOOK_CAR_FAIL'
export const BOOK_CAR_RESET = 'BOOK_CAR_RESET'

export const UPLOAD_CAR_PIC_REQUEST = 'UPLOAD_CAR_PIC_REQUEST'
export const UPLOAD_CAR_PIC_SUCCESS = 'UPLOAD_CAR_PIC_SUCCESS'
export const UPLOAD_CAR_PIC_FAIL = 'UPLOAD_CAR_PIC_FAIL'
export const UPLOAD_CAR_PIC_RESET = 'UPLOAD_CAR_PIC_RESET'